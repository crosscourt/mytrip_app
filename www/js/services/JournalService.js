angular.module('myApp').service('JournalService', function ($q) {
	...
	var service = {
		...
		getEntries: function(){
			...
			try{
				var data = getAllActivities(function(data){
	 	...
	 	getEntriesBetweenDates: function(startDate, endDate){

			var deferred = $q.defer();
			
			try{
				var data = getActivitiesBetweenDates(startDate, endDate, 
					function(data){
						deferred.resolve(data);
					}, 
					function(err){
	...
	function getAllActivities(success, fail) {
		_db.transaction(function (tx) {
			...
			function (tx, results) {
				var activities = parseActivityResult(results);
				success(activities);
			}, 
			function(err) {
	...
	function getActivitiesBetweenDates(startDate, endDate, success, fail) {
		var start = startDate.toYYYYMMDDHHMM();
		var end = endDate.toYYYYMMDDHHMM();

		_db.transaction(function (tx) {
			tx.executeSql('SELECT *, ai.Id as ActivityImageId FROM Activities a JOIN ActivityImages ai on a.Id = ai.ActivityId WHERE a.StartTime BETWEEN ? and ? ORDER BY a.StartTime', [start, end], 
			function (tx, results) {
				var activities = parseActivityResult(results);
				success(activities);
			}, 
	...
	function parseActivityResult(results) {
