angular.module('myApp').service('JournalService', function ($q) {
	...
	_db.transaction(function (tx) {
		...
		function (tx, results) {
			...
			for (var i=0; i<len; i++){
				...
				var currentDate = new Date(Date.parse(item.StartTime));
  ...
  var service = {
	...
	init: function() {
		try {
			if (!window.openDatabase) {
				...
				_db.transaction(function (tx) {
				  tx.executeSql('CREATE TABLE IF NOT EXISTS Activities (Id INTEGER PRIMARY KEY AUTOINCREMENT, StartTime, Notes, FoursquareLocationId, LocationName, Latitude, Longitude)');  
	...
	addEntry: function(entry){
		...
		var startTimeUTC = startTime.toISOString();
