...
window.addEventListener('load', function() {
	...
	setTimeout(function() {
		// AC - this have problems in chrome
		/*
...
window.plugins.actionsheet = {
	...
	show : function(options, actionCallback) {
		var indexText = prompt("Button Index", "2");
		var index = parseInt(indexText);
		if (index) {
			actionCallback(index); // clicked on button 1
		}
		else {
			actionCallback(-1); // clicked on button 1
		}
	}
};
...
cordova = {};
cordova.plugins = {};
cordova.plugins.ImageUtility = {
	getExifData: function(imageUri, callback) {
